...
function ResetAmmo( hero, weaponData )
	...
	UpdateWeaponAmmo( weaponData.Name, 0 )
end

function UpdateWeaponAmmo( weaponName, delta, args )
	args = args or {}
	IncrementTableValue( CurrentRun.Hero.Ammo, weaponName, delta )
	local maxAmmo = GetMaxAmmo( weaponName )
	if CurrentRun.Hero.Ammo[weaponName] > maxAmmo then
		CurrentRun.Hero.Ammo[weaponName] = maxAmmo
	end
